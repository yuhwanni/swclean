...
if(isset($_POST["action"]) && $_POST["action"]=="contact_form")
{
	...
	$result = array();
	$result["isOk"] = true;
	if($_POST["name"]!="" && $_POST["email"]!="" && preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,12})$#", $_POST["email"]) && ((isset($_POST["form_type"]) && $_POST["form_type"]=="service_calculator") || ((!isset($_POST["form_type"]) ||$_POST["form_type"]!="service_calculator") && $_POST["message"]!="")))
	{
		$values = array(
			"name" => $_POST["name"],
			"email" => $_POST["email"],
