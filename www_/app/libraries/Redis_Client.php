...
class Redis_Client {
    ...
    public function __construct ($args = array()) {
        ...
        $this->host = (string) isset($host) ? $host: "localhost";
        $this->port = (int) isset($port) ? $port: "6379";
        $this->timeout = isset($timeout) ? $timeout: "1000";
        $this->persistent = (string) isset($persistent) ? $persistent: "";
        $this->standalone = ! extension_loaded('redis');
        $this->authPassword = isset($password) ? $password: "";
        $this->selectedDb = (int) isset($db) ? $db: "0";
        $this->convertHost();
    }
